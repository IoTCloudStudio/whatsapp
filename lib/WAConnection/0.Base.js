...
class WAConnection extends events_1.EventEmitter {
     ...
     * recieved JSON
     */
    async query({ json, binaryTags, tag, timeoutMs, expect200, waitForOpen }) {
        ...
        if (expect200 && response.status && Math.floor(+response.status / 100) !== 2) {
            ...
            throw new Constants_1.BaileysError(`Unexpected status code in '${json[0] || 'generic query'}': ${response.status}`, { query: json, status: response.status });
        }
        return response;
    }
    /**
    ...
    async waitForConnection(waitForOpen = true) {
        if (!waitForOpen || this.state === 'open')
            return;
    ...
    endConnection() {
        ...
        (_c = this.conn) === null || _c === void 0 ? void 0 : _c.close();
        (_d = this.conn) === null || _d === void 0 ? void 0 : _d.terminate();
        this.conn = null;
        this.lastSeen = null;
